...
namespace thormang3
{
  ...
  double KinematicsDynamics::calcTotalMass(int joint_id)
  {
    ...
    if (joint_id == -1)
    ...
    else
      {
        ...
        thormang3_link_data_[joint_id]->m_tilde_ = mass;
      }
    return mass;
  }
  ...
  Eigen::MatrixXd KinematicsDynamics::calcMassCenter(int joint_id)
  {
    ...
    if (joint_id == -1)
    ...
    else
      {
        ...
        thormang3_link_data_[ joint_id ]->mass_center_ = mc;
      }
  ...
  //matsushima start
  void KinematicsDynamics::calcForwardKinematics4rmc(int joint_id)//とりあえず
  {
    if (joint_id == -1)
      return;
    ...
    if (joint_id == 0)
      {
        ...
        Eigen::Vector3d A;
        A = thormang3_link_data_[46]->position_;
        calcForwardKinematics4rmc(44);
        // thormang3_link_data_[0]->position_ += (A - thormang3_link_data_[46]->position_);
        // thormang3_link_data_[0]->orientation_ = thormang3_link_data_[46]->orientation_.transpose();
        thormang3_link_data_[44]->position_ += (A - thormang3_link_data_[46]->position_);
        thormang3_link_data_[44]->orientation_ = thormang3_link_data_[46]->orientation_.transpose();
      }

    if ( joint_id != 0 && joint_id != 44)
      {
    ...
    calcForwardKinematics4rmc(thormang3_link_data_[joint_id]->sibling_);
    calcForwardKinematics4rmc(thormang3_link_data_[joint_id]->child_);
  }

  void KinematicsDynamics::calcTotalMass(void)
  {
    double mass;

    mass = thormang3_link_data_[0]->mass_ + calcTotalMass(thormang3_link_data_[ 0 ]->sibling_) + calcTotalMass(thormang3_link_data_[0]->child_);

    thormang3_link_data_[0]->m_tilde_ = mass;
  }

  void KinematicsDynamics::calcMassCenter(void)
  {
    Eigen::MatrixXd mc(3,1);

    mc = thormang3_link_data_[ 0 ]->mass_ * ( thormang3_link_data_[ 0 ]->orientation_ * thormang3_link_data_[ 0 ]->center_of_mass_ + thormang3_link_data_[ 0 ]->position_ );
    mc = mc + calcMassCenter( thormang3_link_data_[ 0 ]->sibling_ ) + calcMassCenter( thormang3_link_data_[ 0 ]->child_ );

    thormang3_link_data_[ 0 ]->mass_center_ = mc;
  }

  Eigen::MatrixXd KinematicsDynamics::calcCenterOfMass(int id)
  {
    Eigen::MatrixXd COM(3,1);
    if (id == -1)
      return thormang3_link_data_[0]->mass_center_;//適当すぎる...あとで変更
    else{
      // Eigen::MatrixXd mc = calcMassCenter(id);
      Eigen::MatrixXd mc = thormang3_link_data_[id]->mass_center_;
      double mass;
      // mass = calcTotalMass(id);
      mass = thormang3_link_data_[id]->m_tilde_;
      if(mass == 0)
        COM = mc;
      else
        COM = mc/mass;
      thormang3_link_data_[id]->c_tilde_ = COM;
      calcCenterOfMass(thormang3_link_data_[id]->sibling_);
      calcCenterOfMass(thormang3_link_data_[id]->child_);
    }
    return COM;
  }

  void KinematicsDynamics::calcCenterOfMass(void)
  {
    Eigen::MatrixXd COM(3,1);
    calcMassCenter();
    Eigen::MatrixXd mc = thormang3_link_data_[0]->mass_center_;
    double mass;
    // mass = calcTotalMass(id);
    mass = thormang3_link_data_[0]->m_tilde_;
    if(mass == 0)
      COM = mc;
    else
      COM = mc/mass;

    thormang3_link_data_[0]->c_tilde_ = COM;
    calcCenterOfMass(thormang3_link_data_[0]->sibling_);
    calcCenterOfMass(thormang3_link_data_[0]->child_);
  }

  Eigen::Matrix3d KinematicsDynamics::cross(Eigen::Vector3d x)
  {
  ...
  // Eigen::MatrixXd KinematicsDynamics::calcInertiaB(int id)
  // {
  //   Eigen::Matrix3d I;
  //   Eigen::Matrix3d IB = Eigen::Matrix3d::Zero();
  //   Eigen::Matrix3d rB2ix = Eigen::Matrix3d::Zero();
  //   Eigen::Vector3d com;
  //   com = calcCenterOfMass(0);
    
  //   if (id == -1)
  //     I = Eigen::MatrixXd::Zero(3,3);
  //   else
  //     {
  //       I =
  //         thormang3_link_data_[id]->orientation_
  //         *thormang3_link_data_[id]->inertia_
  //         *thormang3_link_data_[id]->orientation_.transpose();
  //       // rB2ix = cross(link[i].rC - link[info->rootNode].r);
  //       rB2ix = cross(thormang3_link_data_[id]->position_ - com);
  //       IB += I;
  //       // IB += thormang3_link_data_[0]->mass_*rB2ix.transpose()*rB2ix;
  //       IB += calcTotalMass(id)*rB2ix.transpose()*rB2ix;
  //       IB += calcInertiaB(thormang3_link_data_[id]->sibling_,com) + calcInertiaB(thormang3_link_data_[id]->child_,com);
  //     }
  //   return IB;
  // }

  // Eigen::MatrixXd KinematicsDynamics::calcInertiaB(int id, Eigen::Vector3d com)
  // {
  //   Eigen::Matrix3d I;
  //   Eigen::Matrix3d IB = Eigen::Matrix3d::Zero();
  //   Eigen::Matrix3d rB2ix = Eigen::Matrix3d::Zero();
    ...
    if (id != -1)
      {
        I =
          ...
          *thormang3_link_data_[id]->orientation_.transpose();
        // rB2ix = cross(link[i].rC - link[info->rootNode].r);
        rB2ix = cross(thormang3_link_data_[id]->position_ - thormang3_link_data_[44]->position_);
        IB += I;
        // IB += thormang3_link_data_[0]->mass_*rB2ix.transpose()*rB2ix;
        IB += calcTotalMass(id)*rB2ix.transpose()*rB2ix;
        IB += calcInertiaB(thormang3_link_data_[id]->sibling_) + calcInertiaB(thormang3_link_data_[id]->child_);
        thormang3_link_data_[id]->IB_ = IB;
      }
    return IB;
  }

  // Eigen::MatrixXd KinematicsDynamics::calcInertiaC(int id)
  // {
  //   Eigen::Matrix3d IB;
  //   IB = calcInertiaB(id);
  //   // Eigen::Matrix3d rC2Bx = cross(thormang3_link_data_[0]->position_ - calcCenterOfMass(calcMassCenter(id)));
  //   Eigen::Matrix3d rC2Bx = cross(thormang3_link_data_[44]->position_ - calcCenterOfMass(calcMassCenter(id)));
  //   Eigen::Matrix3d IC = IB - calcTotalMass(id) *rC2Bx*rC2Bx.transpose();
  //   return IC;
  // }
  ...
  //     return I;//テキトウ...
  //   }
  //   Eigen::Matrix3d rC2Bx = cross(thormang3_link_data_[44]->position_ - thormang3_link_data_[id]->c_tilde_);
  //   Eigen::Matrix3d IC = thormang3_link_data_[id]->IB_ - thormang3_link_data_[id]->m_tilde_ *rC2Bx*rC2Bx.transpose();
  //   thormang3_link_data_[id]->I_tilde_ = IC;
  //   calcInertiaC(thormang3_link_data_[id]->sibling_);
  //   calcInertiaC(thormang3_link_data_[id]->child_);
  //   return IC;
  // }
  void KinematicsDynamics::calcInertiaC(int id)
  {
    if (id == -1){
      return;
    }
  ...
  void KinematicsDynamics::calcInertiaC(void)
  {
    Eigen::Matrix3d IB;
    IB =  calcInertiaB(0);
    // Eigen::Matrix3d rC2Bx = cross(thormang3_link_data_[0]->position_ - calcCenterOfMass(calcMassCenter(id)));
    Eigen::Matrix3d rC2Bx = cross(thormang3_link_data_[44]->position_ - thormang3_link_data_[0]->c_tilde_);
    Eigen::Matrix3d IC = thormang3_link_data_[0]->IB_ - thormang3_link_data_[0]->m_tilde_ *rC2Bx*rC2Bx.transpose();
    thormang3_link_data_[0]->I_tilde_ = IC;
    calcInertiaC(thormang3_link_data_[0]->sibling_);
    calcInertiaC(thormang3_link_data_[0]->child_);
  }

  Eigen::MatrixXd KinematicsDynamics::pInv(Eigen::MatrixXd A)
  {
    return A.transpose() * (A * A.transpose()).inverse();
  }

  void KinematicsDynamics::rmcInit(void)
  {
    calcForwardKinematics4rmc(0);
    calcTotalMass();
    calcCenterOfMass();
    calcInertiaC();
  }

  void KinematicsDynamics::rmcCalcParam(void)
  {
    calcForwardKinematics4rmc(0);
    calcCenterOfMass();
    calcInertiaC();
  ...
  bool KinematicsDynamics::calcRMC(Eigen::MatrixXd S,
                                   ...
                                   std::vector<Eigen::Matrix<double,6,1>> guzai_i_ref)
  {
    ...
    // calcInertiaC();
    I_tilde = thormang3_link_data_[0]->I_tilde_;
    ...
    c_tilde = thormang3_link_data_[0]->c_tilde_;
    // Eigen::VectorXd th; //現在角度
    // Eigen::VectorXd p_b(3); //現在のベースリンクの位置
    p_b = thormang3_link_data_[44]->position_;
    ...
    A_1 <<
      m_tilde * Eigen::MatrixXd::Identity(3,3) , -m_tilde * cross(r_B_c),
    ...
    for (int i=0; i<MH_id.size(); i++){
      idx_ = findRoute(MH_id_from[i],MH_id_to[i]);
      ...
      for (int j=0; j<idx.size(); j++){
        // m_tilde_j = calcTotalMass(idx(end_id - j));
        // c_tilde_j = calcCenterOfMass(idx(end_id - j));
        // I_tilde_j = calcInertiaC(idx(end_id - j));
        m_tilde_j = thormang3_link_data_[idx(end_id - j)]->m_tilde_;
        c_tilde_j = thormang3_link_data_[idx(end_id - j)]->c_tilde_;
        I_tilde_j = thormang3_link_data_[idx(end_id - j)]->I_tilde_;
        
        r_j = thormang3_link_data_[idx(end_id - j)]->position_;
        m_j = cross(thormang3_link_data_[end_id - j]->joint_axis_) * (c_tilde_j - r_j) * m_tilde_j;
        // m_j = robotis_->cross(a) * (c_tilde_j - r_j) * m_tilde_j;
        h_j = cross(c_tilde_j) * m_j + I_tilde_j * thormang3_link_data_[end_id - j]->joint_axis_;
        M[i].col(end_id - j) = m_j;
        H_[i].col(end_id - j) = h_j;
      }
      H[i] = H_[i] - cross(c_tilde)*M[i];
    
      J[i] = calcJacobian(idx_);

      r_B_i[i] = thormang3_link_data_[MH_id[i]]->position_ - p_b;
      ...
      A_2 <<
        ...
        H[i];
      A_3[i] <<
        Eigen::MatrixXd::Identity(3,3) , - cross(r_B_i[i]),
    ...
    for(int i=0; i<4; i++){
      ...
      for(int j=0; j<idx.size(); j++){
        if ( dth[i](j) >= 5.0 ){//たぶんMAX6.5。とりあえず5にしておく
          ...
          thormang3_link_data_[idx(j)]->joint_angle_ += dth[i](j) * 0.008;
          thormang3_link_data_[idx(j)]->joint_velocity_ = dth[i](j);
          if ( thormang3_link_data_[ idx(j) ]->joint_angle_ >= thormang3_link_data_[ idx(j) ]->joint_limit_max_ ){
            limit_success = false;
            break;
          }else if ( thormang3_link_data_[ idx(j) ]->joint_angle_ <= thormang3_link_data_[ idx(j) ]->joint_limit_min_ ){
            limit_success = false;
            break;
