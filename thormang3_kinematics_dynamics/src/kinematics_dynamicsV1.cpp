...
namespace thormang3
{
  ...
  Eigen::MatrixXd KinematicsDynamics::calcInertiaB(int id)
  {
    ...
    if (id == -1)
    ...
    else
      {
        ...
        rB2ix = cross(thormang3_link_data_[id]->position_ - calcCenterOfMass(calcMassCenter(0)));
        IB += I;
  ...
  Eigen::MatrixXd KinematicsDynamics::calcInertiaC(void)
  {
    //calcForwardKinematics(0);

    Eigen::Matrix3d IB;
    IB = calcInertiaB(0);
    Eigen::Matrix3d rC2Bx = cross(thormang3_link_data_[0]->position_ - calcCenterOfMass(calcMassCenter(0)));
    Eigen::Matrix3d IC = IB - calcTotalMass(0) *rC2Bx*rC2Bx.transpose();
    return IC;
    ...
    //   I = link[i].R*link[i].Iw_C*link[i].R.transpose();
    //   // rB2ix = cross(link[i].rC - link[info->rootNode].r);
    //   rB2ix = cross(thormang3_link_data_[i]->position_ - calcCenterOfMass(thormang3_kd_->calcMassCenter(0)));
    //   IB += link[i].I;
    //   IB += link[i].m*rB2ix.transpose()*rB2ix;      

    // }
    // Eigen::Matrix3d rC2Bx = cross(thormang3_link_data_[0]->position_ - calcCenterOfMass(thormang3_kd_->calcMassCenter(0)));
    // Eigen::Matrix3d IC = IB - calcTotalMass(0) *rC2Bx*rC2Bx.transpose();
  }
